...
:: PowerShell base com cultura fixa en-US
set "PS=powershell -NoProfile -Command "[System.Threading.Thread]::CurrentThread.CurrentCulture='en-US';"
...
for /f "usebackq delims=" %%a in (`
    ...
    "$n='%parcelas%'; try{$n=[int]$n}catch{$n=1}; if($n -lt 1){$n=1}; $n"
`) do set "parcelas=%%a"
...
if "%parcelas%"=="1" (
    for /f "usebackq delims=" %%a in (`
        %PS% [math]::Round([double]%fatorBase% + [double]%fatorPrimeira%, 4)
    `) do set "mdrFlex=%%a"
) else (
    for /f "usebackq delims=" %%a in (`
        %PS% [math]::Round([double]%fatorBase% + [double]%fatorPrimeira% + (([int]%parcelas% - 1) * [double]%fatorAdicional%), 4)
    `) do set "mdrFlex=%%a"
)

:: Calculo da taxa
for /f "usebackq delims=" %%a in (`
    %PS% [math]::Round([double]%bruto% * [double]%mdrFlex% / 100, 2)
`) do set "taxa=%%a"

:: Calculo do valor liquido
for /f "usebackq delims=" %%a in (`
    %PS% [math]::Round([double]%bruto% - [double]%taxa%, 2)
`) do set "liquido=%%a"
